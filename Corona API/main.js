...
function display(regionalData, summaryData,unofficialSummary) {
  regionalData.map((data,index) => {
    let shows = show(index + 1,data.loc, data.confirmedCasesIndian, data.confirmedCasesForeign, data.discharged, data.deaths, data.totalConfirmed);
...
function show(index,state, CasesIndia, CaseSForeing, discharged, deaths, totalConfirmed) {
  let store = `
    <tr>
